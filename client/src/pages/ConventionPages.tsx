...
export function ThemePage() {
  return <><PageHero number="02" eyebrow="Theme" variant="theme" title={<>Shifting the<br />built environment<br /><em>from fragility<br />to resilience.</em></>} text="A Convention premise rooted in climate, local knowledge, material intelligence, and collective practice." /><section className="theme-long"><div className="theme-long-copy">{themeData.paragraphs.map((paragraph, index) => <p key={paragraph} className={index === 0 ? "lede" : ""}>{paragraph}</p>)}</div><aside><p className="eyebrow">Four conversations</p>{themeData.areas.map((area, index) => <div key={area}><span>0{index + 1}</span><p>{area}</p></div>)}</aside></section><RegistrationCTA compact /></>;
}
...
export function BuildToursPage() {
  return <><PageHero number="06" eyebrow="Build Tours" title={<>The region is<br />part of the<br /><em>curriculum.</em></>} text="Technical visits connect the Convention’s themes to working infrastructure, mobility, ecology, and community across Kwale and the coast." /><section className="build-tours-page">{toursData.map((tour, index) => <article key={tour.name}><div><span>{tour.number}</span><h2>{tour.name}</h2></div><p>{tour.summary}</p><Link href="/register" className="text-action">Plan your registration <ArrowUpRight size={18} /></Link>{index !== toursData.length - 1 && <i />}</article>)}</section><RegistrationCTA compact /></>;
}

export function RegisterPage() {
  return <><section className="register-hero"><ArchitecturalGrid tone="dark" /><SectionLabel index="07" label="Registration" /><h1>Secure your<br />place in the<br /><em>Convention.</em></h1><p>AAK’s member event system is the trusted place to select a ticket, provide attendee details, choose your booking type, make payment, and receive confirmation.</p><a className="button-primary light" href={eventData.registrationUrl} target="_blank" rel="noreferrer">Continue to secure registration <ArrowUpRight size={18} /></a></section><section className="register-page"><div className="register-steps"><p className="eyebrow">Your route</p>{["Ticket selection", "Registration details", "Booking type", "Payment", "Confirmation"].map((step, index) => <div key={step}><span>0{index + 1}</span><p>{step}</p></div>)}</div><div className="register-copy"><p className="lede">You’ll complete your reservation in AAK’s secure member event system, where ticket availability, attendee information, booking type, and payment are handled together in one trusted place.</p><div className="register-capabilities">{registrationCapabilities.map((capability) => <p key={capability}>{capability}</p>)}</div><p className="register-note">When you continue, you’ll see current ticket availability and the available payment options. Your payment details are handled within AAK’s secure registration journey.</p><a className="button-outline" href={eventData.registrationUrl} target="_blank" rel="noreferrer">Open registration <ArrowUpRight size={17} /></a></div></section></>;
}
