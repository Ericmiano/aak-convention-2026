...
export function RegisterPage() {
  return <><section className="register-hero"><ArchitecturalGrid tone="dark" /><SectionLabel index="07" label="Registration" /><h1>Secure your<br />place in the<br /><em>Convention.</em></h1><p>AAK’s registration journey guides you from choosing a ticket to receiving your Convention confirmation.</p><a className="button-primary light" href={eventData.registrationUrl} target="_blank" rel="noreferrer">Continue to secure registration <ArrowUpRight size={18} /></a></section><section className="register-page"><div className="register-steps"><p className="eyebrow">Your route</p>{["Ticket selection", "Registration details", "Booking type", "Payment", "Confirmation"].map((step, index) => <div key={step}><span>0{index + 1}</span><p>{step}</p></div>)}</div><div className="register-copy"><p className="lede">Choose your ticket, share the details AAK needs to prepare your place, and complete payment in the trusted registration journey.</p><div className="register-capabilities">{registrationCapabilities.map((capability) => <p key={capability}>{capability}</p>)}</div><p className="register-note">When you continue, you’ll see the latest ticket availability and the payment options available for your booking.</p><a className="button-outline" href={eventData.registrationUrl} target="_blank" rel="noreferrer">Open registration <ArrowUpRight size={17} /></a></div></section></>;
}
